@extends('layouts/app')

@section('styles')
<style type="text/css">
	th{
		font-weight: bold;
	}	
	

</style>
	
@endsection

@section('content')

<!-- Body -->
<div class="container small pt-3 pb-5">
	<div class="row">
		<h3 class="card-header text-white text-center mx-auto d-block" >LOCAL INSTITUTIONAL CAPABILITY</h3>
	</div>
	<div class="row mt-3">
		<div class="col-sm-12">
			<section>
				<!-- <h5 class="mb-1 mt-1">ROADS</h5> -->
				<p>
					Among the school age population, the elementary registered a quite high participation rate of 93.95%. one reason for this that it did not hit the l00% participation rate is that students enrolled in other municipalities were not included in the report from the schools concerned the report relied mainly on the number of enrolled students in the different schools. The high school level on the other hand, reported a very low participation rate of 85.63%. The reason for this is that the Municipality belongs to a mountainous area and that some indigenous school age population did not enroll in high school but preferred to stop because of the distance of school in the barangay. Some goes to other school in the adjacent municipalities of La Union unlike in the elementary level that almost all barangay has school building established near their residences.
				</p>				
			</section>

			<section>
				<h3>
					LOCAL FISCAL MANAGEMENT
				</h3>
				<h5 class="mb-1 mt-1">
					STATUS OF FINANCIAL
				</h5>
				<p>
					There are nineteen (19) schools found in Bagulin, La Union, Fourteen (14) of which are Public Elementary School and five (5) Public High School. Tables below shows the population of enrollment in elementary and high school.
				</p>				
			</section>



			<section class="mt-5">
				<center>
					<strong>
						SUMMARY OF RECEIPTS AND EXPENDITURES  2015, 2016, 2017, 2018


						<br />
						<small>Source: Accounting Office, Budget and treasury</small>
					</strong>
				</center>

				<table class = "table table-condensed table-hover table-bordered table-striped table-sm">
					<tbody>
						<tr>
							<td>
								<p>Transactions</p>
							</td>
							<td>
								<p>2015</p>
							</td>
							<td>
								<p>2016</p>
							</td>
							<td>
								<p>2017</p>
							</td>
							<td>
								<p>2018</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Total Receipts</p>
							</td>
							<td>
								<p>58,747,655.97</p>
							</td>
							<td>
								<p>74,268,636.50</p>
							</td>
							<td>
								<p>64,892,616.53</p>
							</td>
							<td>
								<p>102,874,012.00</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Total Expenditures</p>
							</td>
							<td>
								<p>40,121,937.29</p>
							</td>
							<td>
								<p>44,732,343.10</p>
							</td>
							<td>
								<p>45,811,549.67</p>
							</td>
							<td>
								<p>56,784,507.34</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Ending Cash Balance</p>
							</td>
							<td>
								<p><strong>18,625,718.68</strong></p>
							</td>
							<td>
								<p><strong>29,536,293.40</strong></p>
							</td>
							<td>
								<p><strong>19,081,066.86</strong></p>
							</td>
							<td>
								<p><strong>46,089,504.66</strong></p>
							</td>
						</tr>
					</tbody>
				</table>

				
			</section>

			<section>

				<h5 class="mb-1 mt-1">
					MUNICIPAL REVENUES BY SOURCE
				</h5>
				<p>
					In terms of sources of revenues over a period of three years, there is no regular pattern that can be noticed.
				</p>				
			</section>


			<section class="mt-5">
				<center>
					<strong>
						MUNICIPAL REVENUES BY SOURCE
						2015, 2016, 2017 & 2018
						<br />
						<small>Source: Accounting Office</small>
					</strong>
				</center>

				

				<table class = "table table-condensed table-hover table-bordered table-striped table-sm">
					<tbody>
						<tr>
							<td>
								<p>Major Revenue Sources</p>
							</td>
							<td>
								<p>2015</p>
							</td>
							<td>
								<p>2016</p>
							</td>
							<td>
								<p>2017</p>
							</td>
							<td>
								<p>2018</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Operating &amp; Misc. Revenues</p>
							</td>
							<td>
								<p>931,512.82</p>
							</td>
							<td>
								<p>1,088,239.88</p>
							</td>
							<td>
								<p>1,259,347.25</p>
							</td>
							<td>
								<p>1,381,699.17</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Tax Revenues</p>
							</td>
							<td>
								<p>1,313,351.15</p>
							</td>
							<td>
								<p>1,429,687.62</p>
							</td>
							<td>
								<p>1,774,725.28</p>
							</td>
							<td>
								<p>1,918,649.83</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Borrowings</p>
							</td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						<tr>
							<td>
								<p>Total</p>
							</td>
							<td>
								<p>2,244,863.97</p>
							</td>
							<td>
								<p>2,517,927.50</p>
							</td>
							<td>
								<p>3,034,072.53</p>
							</td>
							<td>
								<p>3,300.349.00</p>
							</td>
						</tr>
					</tbody>
				</table>

				
			</section>



			<section class="mb-1 mt-1">

				<!-- <h5 class="mb-1 mt-1">
					MUNICIPAL REVENUES BY SOURCE
				</h5> -->
				<p>
					The data below shows the proportion of local revenues to total Municipal Income. The table shows that the biggest part of the income derived from the Internal Revenue Allotment (IRA) or equivalent to 96.23% of the Total income for the year 2017. Only 3.77% of the Municipality’s income is locally generated from all sources. This indicates that the LGU’s operation is dependent on the Internal Revenue Allotment (IRA). The challenge therefore is for the LGU to find ways to increase its local revenues.
				</p>				
			</section>


			<section class="mt-5">
				<center>
					<strong>
						EXTENT OF FISCAL AUTONOMY
						2015, 2016, 2017,2018

						<br />
						<small>Source: Accounting Office</small>
					</strong>
				</center>

				<table class = "table table-condensed table-hover table-bordered table-striped table-sm">
					<tbody>
						<tr>
							<td>&nbsp;</td>
							<td>
								<p>2015</p>
							</td>
							<td>
								<p>2016</p>
							</td>
							<td>
								<p>2017</p>
							</td>
							<td>
								<p>2018</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Locally Generated Revenues (all sources)</p>
							</td>
							<td>
								<p>2,244,863.97</p>
							</td>
							<td>
								<p>2,517,927.50</p>
							</td>
							<td>
								<p>2,423,178.79</p>
							</td>
							<td>
								<p>2,668,409.34</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Internal Revenue Allotment</p>
							</td>
							<td>
								<p>49,768,921.00</p>
							</td>
							<td>
								<p>54,776,048.00</p>
							</td>
							<td>
								<p>61,858,544.00</p>
							</td>
							<td>
								<p>66,363,520.00</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Share from Tobacco excise Tax</p>
							</td>
							<td>
								<p>6,733,871.00</p>
							</td>
							<td>
								<p>16,974,661.00</p>
							</td>
							<td>&nbsp;</td>
							<td>
								<p>33,210,143.00</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Total</p>
							</td>
							<td>
								<p><strong>58,747,655.97</strong></p>
							</td>
							<td>
								<p><strong>74,268,636.50</strong></p>
							</td>
							<td>
								<p><strong>64,281,722.79</strong></p>
							</td>
							<td>
								<p><strong>102,242,072.34</strong></p>
							</td>
						</tr>
					</tbody>
				</table>
			</section>


			<section class="mb-1 mt-1">

				<h5 class="mb-1 mt-1">
					ACTUAL EXPENDITURES BY GENERAL ACCOUNT
				</h5>
				<p>
					
					The data below shows the pattern of expenditures by the Municipality over a period of three years. 

				</p>				
			</section>

			<section class="mt-5">
				<center>
					<strong>
						ACTUAL EXPENDITURES
						2015, 2016, 2017, 2018


						<br />
						<small>Source: Accounting Office</small>
					</strong>
				</center>

				<table class = "table table-condensed table-hover table-bordered table-striped table-sm">
					<tbody>
						<tr>
							<td>
								<p>Actual Expenditures</p>
							</td>
							<td>
								<p>2015</p>
							</td>
							<td>
								<p>2016</p>
							</td>
							<td>
								<p>2017</p>
							</td>
							<td>
								<p>2018</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Personnel Services</p>
							</td>
							<td>
								<p>22,396,944.76</p>
							</td>
							<td>
								<p>26,566,092.97</p>
							</td>
							<td>
								<p>26,383,371.39</p>
							</td>
							<td>
								<p>31,789,591.50</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>M O O E</p>
							</td>
							<td>
								<p>15,707,790.31</p>
							</td>
							<td>
								<p>15,947,909.63</p>
							</td>
							<td>
								<p>17,098,156.80</p>
							</td>
							<td>
								<p>21,452,242.39</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Capital Outlay</p>
							</td>
							<td>
								<p>13,459,670.53</p>
							</td>
							<td>
								<p>3,911,596.65</p>
							</td>
							<td>
								<p>9,311,934.79</p>
							</td>
							<td>
								<p>14,420,166.65</p>
							</td>
						</tr>
						<tr>
							<td>
								<p>Total</p>
							</td>
							<td>
								<p>51,600,405.60</p>
							</td>
							<td>
								<p>46,425,599.10</p>
							</td>
							<td>
								<p>52,793,462.98</p>
							</td>
							<td>
								<p>67,622,000.54</p>
							</td>
						</tr>
					</tbody>
				</table>

				

				
			</section>






		</div>
	</div>
</div>

@endsection